...
local function main()
  ...
  local ok,
    ...
    pcall(
    function()
      return torch.load(opt.loss_network)
    end
  )
  if not ok then
  ...
  print(loss_net)
  local style_layers,
    style_weights = utils.parse_layers(opt.style_layers, opt.style_weights)
  local content_layers,
    content_weights = utils.parse_layers(opt.content_layers, opt.content_weights)
  local deepdream_layers,
    deepdream_weights = utils.parse_layers(opt.deepdream_layers, opt.deepdream_weights)
  local crit_args = {
    cnn = loss_net,
  ...
  local H,
    W = content_image:size(2), content_image:size(3)
