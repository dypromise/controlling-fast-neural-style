...
function main()
  ...
  opt.content_layers,
    opt.content_weights = utils.parse_layers(opt.content_layers, opt.content_weights)
  opt.style_layers,
    opt.style_weights = utils.parse_layers(opt.style_layers, opt.style_weights)
  ...
  if opt.percep_loss_weight > 0 then
    ...
    if opt.task == "style" then
      ...
      local H,
        W = style_image:size(2), style_image:size(3)
  ...
  local params,
    grad_params = model:getParameters()
  ...
  local function f(x)
    ...
    local x,
      y = loader:getBatch("train")
    x,
      y = x:type(dtype), y:type(dtype)
    ...
    if opt.style_target_type == "guided_gram" then
      local N,
        H,
        W = y[1]:size(1), y[1]:size(3), y[1]:size(4)
  ...
  for t = 1, opt.num_iterations do
    ...
    local _,
      loss = optim.adam(f, params, optim_state)
